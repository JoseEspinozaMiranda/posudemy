...
class Ventas extends Controller
{
                ...
                if (empty($VerificarCaja['monto_inicial'])) {
                    ...
                    if ($venta > 0) {
                        foreach ($datos['productos'] as $producto) {
                            ...
                            $movimento = 'Venta N°: ' . $venta;
                            $cantidad = $producto['cantidad'];
                            $this->model->registrarMovimiento($movimento, 'Entrada',  $cantidad, $nuevaCantidad, $producto['id'], $this->id_usuario);
                        }
                        if ($metodo == 'CREDITO') {
    ...
    public function enviarCorreo($idVenta)
    {
        ...
        $data['empresa'] = $this->model->getEmpresa();
        $data['venta'] = $this->model->getVenta($idVenta);
        // $data = $this->model->getVenta($idVenta);
        // $empresa = $this->model->getEmpresa();
        ob_start();
        $data['title'] = 'Reporte';
        ...
        if (!empty($data)) {
            ...
            try {
                ...
                $mail->send();  
                    $res = array('msg' => 'CORREO ENVIADO CON DATOS DE LA VENTA...', 'type' => 'success');
